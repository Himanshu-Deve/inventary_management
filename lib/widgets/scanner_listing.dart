...
class ScanListingScreen extends StatefulWidget {
  final dynamic bloc; // instead of final bloc;
  final int? selectId;
  final bool? isNew;
...
// Inside _ScanListingScreenState
  ...
  Widget build(BuildContext context) {
    if (widget.bloc is MachineInBloc) {
      return BlocProvider<MachineInBloc>.value(
        value: widget.bloc as MachineInBloc,
        child: scanBuild(),
      );
    }

    return BlocProvider<MachineOutBloc>.value(
      value: widget.bloc as MachineOutBloc,
      child: scanBuild(),
    );
  }
  ...
  Scaffold scanBuild() {
    return Scaffold(
      ...
      body: MultiBlocListener(
        listeners: [
          if (widget.bloc is MachineInBloc)
            BlocListener<MachineInBloc, MachineInState>(
              listener: (context, state) {
                if (state.saveSuccess != null) {
                  AppUtils.showCustomPopup(
                    title: "Success",
                    content: Text(state.saveSuccess.toString()),
                  );
                }
              },
            ),
          if (widget.bloc is MachineOutBloc)
            BlocListener<MachineOutBloc, MachineOutState>(
              listener: (context, state) {
                if (state.saveSuccess != null) {
                  AppUtils.showCustomPopup(
                    title: "Success",
